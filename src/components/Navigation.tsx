  ...
  return (
    ...
    <nav className="fixed max-w-md bottom-0 left-50 bg-white border-t border-gray-200 py-2" style={{'width': '100%'}}>
      <div className="flex justify-around items-center">
        <Link to="/" className={`flex flex-col items-center ${location.pathname === '/' ? 'text-red-500' : 'text-gray-500'}`}>
          <Home size={24} />
          <span className="text-xs mt-1">Home</span>
        </Link>
        <Link to="/attendance" className={`flex flex-col items-center ${location.pathname === '/attendance' ? 'text-red-500' : 'text-gray-500'}`}>
          <Calendar size={24} />
          <span className="text-xs mt-1">Attendance</span>
        </Link>
        <div className="relative -top-5"
        >
          <div className="flex flex-col items-center justify-center w-16 h-16 rounded-full bg-red-500 text-white">
            {
              isCheckedIn ?
            ...
            <span className="text-xs text-red-500 font-bold mt-1">{isCheckedIn ? 'Check Out' : 'Check In'}</span>
        </div>
        <Link to="/form" className={`flex flex-col items-center ${location.pathname === '/form' ? 'text-red-500' : 'text-gray-500'}`}>
          <FileText size={24} />
          <span className="text-xs mt-1">Form</span>
        </Link>
        <Link to="/settings" className={`flex flex-col items-center ${location.pathname === '/settings' ? 'text-red-500' : 'text-gray-500'}`}>
          <Settings size={24} />
